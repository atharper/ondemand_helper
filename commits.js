      ...
      if (build != undefined) {
        ...
        if ($(this).find('.copy-stuff').length === 0)
        {
          ...
          cell.append('<div class="commit-links-group BtnGroup copy-stuff">' +
           ...
           '</path></svg></button><a href="' + build.infoLink + '" aria-label="Get Build Information" class="sha btn btn-outline tooltipped tooltipped-sw" style="width:125px;text-align:center;' + color + '" rel="nofollow">' + buildNumber + '</a></div>');
        }
      }
    }
  });
